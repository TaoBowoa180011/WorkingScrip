...
sys.path.insert(0, ".")
from .paddle_utils import preprocess,postprocess


def predict(args, predictor, img=''):
    ...
    classes, scores = postprocess(output, args)

    #dynaconf_config_paddle = settings.get('paddle', {})
    #classifiy_classes_ultra = dynaconf_config_paddle.PADDLE_CLASSES_ULTRASONIC
    classifiy_classes_sink = ['hand','nohand']
    classes = [classifiy_classes_sink[cls] for cls in classes]
    # added
    #if debug:
